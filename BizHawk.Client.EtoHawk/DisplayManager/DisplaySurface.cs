...
namespace BizHawk.Client.EtoHawk
{
	...
	public unsafe class DisplaySurface : IDisposable
	{
		...
		/// </summary>
        public Eto.Drawing.Graphics GetGraphics()
		{
			ToBitmap();
            return new Eto.Drawing.Graphics(bmp);
		}
		...
		public unsafe void ToBitmap(bool copy=true)
		{
			...
			if (bmp == null)
			{
                bmp = new Bitmap(Width, Height, PixelFormat.Format32bppRgba);
			}
			...
			if (copy)
			{
                using (BitmapData bmpdata = bmp.Lock())
                {
                    int w = Width;
                    int h = Height;
                    int stride = bmpdata.ScanWidth / 4;
                    int* bmpbuf = (int*)bmpdata.Data.ToPointer();
